...
struct ProjectChimeraApp: App {
    ...
    @StateObject private var gameState = ChimeraGameState()
    
    init() {
        // Initialize game state with seed data
    ...
    var body: some Scene {
        WindowGroup {
            ZStack {
                ...
                // Toast overlay
                ToastOverlay(toastCenter: gameState.toastCenter)
            }
                .modelContainer(for: [
                    User.self,
                    Task.self,
                    ...
                    Resource.self,
                    Building.self,
                    PlayerExpedition.self,
                    PlayerExpeditionReport.self
                ])
        }
    }
}
